...
export class CreateErrorLogsTable1742348500000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TYPE "error_logs_severity_enum" AS ENUM ('low', 'medium', 'critical')
    `);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP INDEX IF EXISTS "IDX_error_logs_fingerprint"`);
    await queryRunner.query(`DROP INDEX IF EXISTS "IDX_error_logs_severity_createdAt"`);
    await queryRunner.query(`DROP INDEX IF EXISTS "IDX_error_logs_userId_createdAt"`);
